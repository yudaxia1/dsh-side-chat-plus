...
export function eventPlainText(data) {
...
/** 截断到上限，超长时保留头尾并标注省略。 */
export function boundText(text, max = ANCHOR_TEXT_MAX) {
...
export function findMessageInLog(events, messageId) {
...
export function adjacentUserContext(events, anchorSeq) {
  let lastUser = null
  for (const event of events) {
    if (event === null || typeof event !== 'object') continue
...
export function recentMessages(events, limit = RECENT_MESSAGE_LIMIT) {
  const out = []
...
export function modelConfigFromLog(events) {
...
export function systemPrompt(anchorSnapshot) {
  const lines = [
...
export function buildModelMessages(record, anchorSnapshot, newText) {
  const messages = []
...
export function summarizePrompt(record) {
  const lines = [
...
/** 默认侧聊标题（由锚点推导）。 */
export function defaultTitle(anchorSnapshot) {
...
/** 创建侧聊核心工厂。 */
export function createSideChatCore(deps) {
