...
dsh 0.1.6-alpha.2 起官方支持把**子代理会话**开在右栏（PR #4417）。两者定位不同，可同时安装、互不冲突：

| | 官方（alpha.2 内置） | 本插件 |
|---|---|---|
| 会话从哪来 | agent 自己 spawn 的子代理（subagent 工具） | 用户主动开，任何主会话随时可开 |
| 定位 | 查看/跟踪子代理的执行过程 | 主会话旁边的自由问答（/btw 式） |
| 权限 | 跟随子代理自身配置；one-shot 的只能看 | 默认只读（沙箱强制 + 审批关闭），可一键关掉 |
| 带回主会话 | ❌ 没有 | ✅ ↗ 一键晋升到主会话输入框草稿 |
| 主会话背景 | 子代理的任务上下文（spawn 时给的） | ✅ 按需 side_chat_context 工具主动检索 |
| 生命周期 | 跟随子代理 | ✅ 用户控制：保留/删除/恢复、关闭行为可记 |
| 会话列表 | 子代理出现在左侧（分组里） | ✅ 归档隐藏，不占列表 |
| 模型 | 子代理继承 | ✅ 原生选择器独立选 |
| 预设 | 跟随 | ✅ standard / ptc / minimal / cordis 可选 |

一句话：官方做的是"围观 agent 的分身"，本插件做的是"用户的第二张嘴"。
